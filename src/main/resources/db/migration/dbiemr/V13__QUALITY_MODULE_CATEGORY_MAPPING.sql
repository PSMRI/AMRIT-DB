...
DROP VIEW IF EXISTS dbiemr.v_get_qualityaudit_sectionquestionairevalues;

CREATE 
    ALGORITHM = UNDEFINED 
    DEFINER = `piramaldev`@`%` 
    SQL SECURITY DEFINER
VIEW `dbiemr`.`v_get_qualityaudit_sectionquestionairevalues` AS
    SELECT 
        UUID() AS `id`,
        `t1`.`id` AS `sectionId`,
        ...
        `t2`.`Role` AS `Role`
    FROM
        ((`dbiemr`.`m_qualityauditsection` `t1`
        JOIN `dbiemr`.`m_qualityauditquestionnaire` `t2` ON ((`t1`.`id` = `t2`.`Sectionid`)))
        JOIN `dbiemr`.`m_qualityauditquestionnairevalues` `t3` ON ((`t2`.`id` = `t3`.`QuestionID`)))
    WHERE
        ((`t1`.`Deleted` IS FALSE)
