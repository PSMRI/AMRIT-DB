...
DELIMITER $$
CREATE PROCEDURE sp_v105_assert_synctabledetail_row(IN p_table_name VARCHAR(100))
BEGIN
    DECLARE row_count INT;
SELECT COUNT(*) INTO row_count FROM m_synctabledetail WHERE TableName = p_table_name;
IF row_count = 0 THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'V105 aborted: no m_synctabledetail row found for this TableName -- cannot register external_order_id in sync column mapping';
END IF;
END$$
DELIMITER ;
...
CALL sp_v105_assert_synctabledetail_row('tb_diagnostic_result');
...
CALL sp_v105_assert_synctabledetail_row('tb_diagnostic_document');
...
DROP PROCEDURE sp_v105_assert_synctabledetail_row;