...
--
-- V22 added them to the two db_identity tables of the first cut. This script extends that
...
--   * plus the db_identity tables of the first cut, i_householddetails /
--     i_beneficiarydetails_rmnch, which V22 already covers - they are listed
--     again so this script is complete on its own.
--
-- Every other table in db_identity is deliberately left alone. A table that
-- does not sync has nothing to track.

-- The configuration itself lives in db_iemr.m_synctabledetail and carries rows
-- for both schemas; this script picks the SchemaName = 'db_identity' ones. The
-- rollout report is shared with the db_iemr rollout, in
-- db_iemr.t_downsynccolumnrollout, so there is one place to look.
--
...
-- as is running this after V22. Re-run it after adding rows to
...
-- The two first-cut tables are already covered by V22; naming them here makes
-- this script correct even if V22 was skipped.
CALL amrit_downsync_columns_for_synced_tables('db_identity', 'i_householddetails,i_beneficiarydetails_rmnch');
